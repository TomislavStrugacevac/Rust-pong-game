...
// rednder paddles and the ball
    fb render(&mut self, args: &RenderArgs) {
     ...
     self.gl.draw(args.viewport(), |c, gl| {
        ...
        rectangle( FOREGROUND, 
                    left, 
                    c.transform.trans(-40.0, left_pos),
                    gl);
    ...
    fn update (&mut self, _args: &UpdateArgs) {
        ...
        // logic for ball passing the right paddle 
        if self.ball_x > 502 {
            
            self.vel_x = -self.vel_x; //change direction of the ball
        ...
        //logic for ball passing the left paddle
        if self.ball_x < 1 {
        ...
        if ( self.ball_y < 1 ) || ( self.ball_y > 332 ) {
